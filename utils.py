...
@st.cache_data(ttl=3600) # On garde le token en mémoire pendant 1h pour ne pas spammer le serveur
...
def recuperer_donnees_atmo(endpoint="/api/v2/data/indices/atmo"):
    ...
    url = f"https://admindata.atmo-france.org{endpoint}"
    ...
    try:
        ...
        if reponse.status_code == 200:
            ...
            return {"erreur": f"Code serveur {reponse.status_code}"}
    except Exception as e:
        return {"erreur": str(e)}

# --- ALGORITHMES DE NORMALISATION (Existants) ---
...
# --- EXPERTISE SANTÉ ATMO FRANCE ---
def generer_conseils(meteo, risque_pollen, risque_pollution):
    """Génère des conseils basés sur la documentation officielle Atmo."""
    conseils = []
    
    # Conseils Pollen & Météo
    if meteo == "Pluvieux":
    ...
    # Conseils Pollution & Air intérieur
    if risque_pollution >= 3:
        conseils.append("⚠️ **Alerte :** La pollution de l'air exacerber les allergies aux pollens et te rendre plus sensible.")
